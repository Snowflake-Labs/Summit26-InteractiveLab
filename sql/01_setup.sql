...
-- Account default: PAT/network evaluation for lab accounts (service user keeps ARCADE_KEYPAIR_POLICY above).
ALTER ACCOUNT SET AUTHENTICATION POLICY pat_bypass_policy;
...
-- Streaming role: Snowpipe Streaming into the interactive table.
-- GRANT INSERT is not valid on INTERACTIVE_TABLE (SQL compilation error); the ingest role
-- must own the target table so streaming APIs can load rows.
...
GRANT OWNERSHIP ON TABLE ARCADE_DB.PUBLIC.ARCADE_SCORES      TO ROLE ARCADE_STREAMING_ROLE;
...
GRANT ROLE ARCADE_LAB_READER TO ROLE ACCOUNTADMIN;
